...
class DrumSig implements AudioSignal {
  float f=0, a=0, phase=0, noiseAmp=0;
  int type = 0; // 0: Kick, 1: Snare
  int endTime=0;

  void on(float note, float amp, int dur) {
    a = amp * 0.8;
    endTime = millis() + dur;
    phase = 0;
    if (note <= 60) { // 低い音はキック
  ...
  void generate(float[] samp) {
    for (int i = 0; i < samp.length; i++) {
      if (millis() < endTime) {
        if (type == 0) { // Kick: 150Hz -> 50Hzへ急降下
          samp[i] = sin(phase) * a;
          phase += TWO_PI * f / out.sampleRate();
          f *= 0.999; 
          if(f < 50) f = 50;
        } else { // Snare: 180Hz + Noise
          float tone = sin(phase) * 0.4;
          float snap = (random(2.0)-1.0) * noiseAmp;
          samp[i] = (tone + snap) * a;
          phase += TWO_PI * 180.0 / out.sampleRate();
          noiseAmp *= 0.999;
        }
        a *= 0.9998; // 打楽器なので常に減衰
      } else {
